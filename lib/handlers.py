...
async def check_user_wishes(call: CallbackQuery, data, bot: CustomBot):
    ...
    if not usr:
        await bot.send_message(call.from_user.id, CALLBACK_TEXTS.user_unavailable)
    elif not usr.wish_string:
...
async def user_wishes_keyboard_shift(call: CallbackQuery, data, bot: CustomBot):
    try:
        chatid, offset = call.data[16:].split()
        chatid = int(chatid)
        offset = int(offset)
    except:
        import traceback
        traceback.print_exc()
        return
    participants = await get_group_participants(chatid)
    userids = [(i+1, m.id) for i, m in enumerate(participants)]
